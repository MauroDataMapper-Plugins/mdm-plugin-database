...
class RemoteDatabaseImportAndExporter {
    ...
    void outputRuntimeArgs() {
        ...
        log.warn("Running with JVM args:\n  " + arguments.join('\n  '))
    }
    ...
    List<DataModel> importDatabases(Properties loadedProperties, CatalogueUser catalogueUser) {
        AbstractDatabaseDataModelImporterProviderService databaseImporter =
                ...
                if (folderPath) {
                    ...
                    if (folderJson) {
                        ...
                        dataModels.each { dataModel ->
                            ...
                            log.
                                    info('Using JSON importer {}.{} (v{})', jsonImporterInfo.namespace, jsonImporterInfo.name,
                                         jsonImporterInfo.version)
                            DataModelFileImporterProviderServiceParameters importerPluginParameters =
                            ...
                            result =
                                    post(host,
                                         "/dataModels/import/${jsonImporterInfo.namespace}/${jsonImporterInfo.name}/${jsonImporterInfo.version}",
    ...
    private static String getDomainTemplateUri(domain) {
        String domainName = domain.class.simpleName
        "/${domainName.uncapitalize()}/_${domainName.uncapitalize()}.gson"
    }

    private static Map getRenderModel(domain) {
        Map<String, Object> map = [:]
        map["${domain.class.simpleName.uncapitalize()}" as String] = domain
        map
    }

    private static String prettyPrint(String jsonBody) {
        try {
    ...
    private static void shutdownGorm() throws IOException {
